...
| **The PREVIEW RENDERING GUARD in `src/gramps_live_api/core/schema.py`** (`_is_unrenderable`, added for #33 -- a different guard from `pii_guard`, recorded here because this is where residuals are kept) **gives verdicts that depend on the interpreter's Unicode version.** The same operation can preview on one supported interpreter and refuse on another: U+0890 is `Cn` under Python 3.10's UCD 13.0.0 and `Cf` under 3.11's 14.0.0 and 3.12's 15.0.0, so 3.10 emits a formatting character that 3.11 and 3.12 refuse. **The older interpreter is the permissive side**, measured rather than inferred -- 3.12 guards **9** code points 3.10 does not (U+0890, U+0891 and U+13439-U+1343F, all `Cf`) and 3.10 guards none 3.12 does not. | Accepted because both deterministic alternatives are enumerations this project refuses everywhere else -- the same grounds every other enumeration argument in this table rests on. Pinning an explicit set of format characters is stable across interpreters and fails **open** on whatever the standard assigns next; inverting to a published safe-to-display set is that enumeration pointed backwards, stable too and failing **closed** on legitimate text nobody listed. Deriving the class from the running UCD is the trade taken, and the arithmetic is why the `Cn` exclusion stays: **the exclusion does not remove version dependence, it shrinks it from 5,327 code points to 9 and flips its direction from fail-closed to fail-open.** Had `Cn` been guarded, the 5,327 code points assigned between UCD 13.0.0 and 15.0.0 would flip verdict with the OLDER interpreter refusing -- legitimate data blocked, on a far wider set. Shrinking it is why the exclusion stays; the flip is why this row exists rather than a fix. Figures measured on Windows 11 x86-64 CPython 3.10.20 / 3.11.15 / 3.12.13, each invoked by absolute path. |
