...
**Removed: the preview rendering guard's version-dependence row.** It said `_is_unrenderable` in
...
in `schema.py`'s own costs block, where a `core/schema` residual belongs; it is not moved into
this table. **A residual row left standing reads as current when it is not** -- the same reason
