...
The core leg's `Types` step runs `mypy src/gramps_agent_data_entry` rather than `mypy src`, because the MCP
...
**Then confirm it took:** `python -m gramps_agent_data_entry check` reports a `push gate`
...
```sh
PYTHONPATH=src python -m gramps_agent_data_entry.core.pii_guard .
```

```powershell
# Windows PowerShell -- there is no inline environment prefix
$env:PYTHONPATH = "src"; python -m gramps_agent_data_entry.core.pii_guard .
```
...
```sh
PYTHONPATH=src python -m gramps_agent_data_entry.core.pii_guard --range origin/main..HEAD .
```
...
`src/gramps_agent_data_entry/core/pii_guard.py` enforces two properties over every file in the checkout:
...
```sh
PYTHONPATH=src python -m gramps_agent_data_entry.core.pii_guard --range main..HEAD .
```
...
| **#17's criterion 3 is not met: a drawing holding TWO or more empty attributed prose elements is reported.** One of them is worth 2 and escapes; two reach the threshold of 4 together. | Recorded with the change that made an attributed prose row's weight depend on what it encloses. ⚠️ **Nothing here was reverted and no false positive returned.** The drawing exemption was never extended to the attributed pass on this branch -- see the row above -- so criterion 3 was not met before this change either. What moved is the **count**, measured on a **Windows 11 Pro 10.0.26100 development machine, CPython 3.12.13 via `.venv\Scripts\python.exe`**: one empty label in a drawing went from **4 -- reported** to **2 -- clean**, and two of them from **8** to **4**, reported both before and after. Closing it means either charging an element that encloses nothing more than one enclosing a space -- which is #17 itself, and the thing this change exists to stop -- or lowering the threshold, whose own margins are measured beside `_GRAMPS_SCORE_THRESHOLD` and are not this change's to spend. **A chart whose labels carry words or numbers is untouched**, because the filled pass claims those and keeps its short-content gate; what escapes is only the spelling that encloses nothing at all. Asserted by test in three arms -- two of every label spelling outside a drawing is a finding, one of each inside it is clean, and two of each inside SPLITS according to whether the filled pass can read it. The deny-list is the backstop, here as elsewhere. |
| **A prose element whose content the FILLED pass cannot read is charged what an empty one is charged, and not what its content is worth.** A self-closing `<text x=…/>`, its paired-empty spelling, and a wrapper whose content opens with a nested start tag are all measured as enclosing nothing. | **The price of the change above, and it is stated in both directions rather than as a residual that merely remains.** The change removes a false positive -- an empty attributed prose element charged prose weight for enclosing nothing, which is what #17 was filed about -- and **pays for it with a fail-open**: an element whose content the filled pattern cannot delimit is charged as though there were none. The pattern's content alternative admits neither `<` nor any markup node that ends where an element does, so it cannot reach such an element's closing tag and never claims it. Direction is **fail-open by 2**, below the threshold on its own; two are caught, which is **the ceiling already recorded beside the prose floor** rather than a new one -- exactly what a bare short note does, and the same ceiling the comment row above concedes. The obvious repair is to teach the attributed pass to measure content it cannot delimit, which is a parser, refused repeatedly here on the standing grounds. The deny-list is the backstop. |
| **ONE prose wrapper inside a drawing is not caught** -- a `<note handle=…>` start tag enclosing a name, a date of birth and a place as nested short elements, all inside an `<svg>`. | ⚠️ **A fail-open this change INTRODUCES rather than one it inherits, and the row says so because that is what it is.** Before the change such a wrapper scored **4** -- the threshold exactly -- and was reported; after it, **2**, and it is clean. Two are worth 4 and are reported, three 6; with no drawing around it one is worth **8** and reported. Measured on a **Windows 11 Pro 10.0.26100 development machine, CPython 3.12.13 via `.venv\Scripts\python.exe`**. The single-wrapper figure is below the threshold and the scorer returns nothing there, so it is **not directly observable and was derived two independent ways that agree**: the marginal copy (three at 6 minus two at 4) and the same one-wrapper drawing beside a carrier that already clears the threshold (6 minus 4). Both give 2. ⚠️ **The mechanism, stated the way a repair would need it rather than the way it looks:** the wrapper's own contribution is **drawing-independent** -- derived by test over every prose row of the vocabulary rather than exampled -- and what the drawing removes is its **children**, ordinary short prose which the filled pass does claim and whose exemption then suppresses them. So **a repair that distinguished an unreadable-content wrapper from an empty one would not close this**, because the wrapper is charged its 2 either way. The deny-list is the backstop. **Where the answer belongs is issue #33's rendering boundary, and #33 is MERGED** -- what it built is the structural guard over what `preview()` **emits**, which is not the *"is this really a chart label"* question; that question is further work **at** that boundary rather than something #33 has already answered. This is the third consecutive round to leave a residual in this one mechanism, and all three name the same destination. |
...
**Removed: the approval render guard's version-dependence row.** It said `_is_unrenderable` in
`src/gramps_live_api/core/schema.py` gave verdicts that depended on the interpreter's Unicode
version -- the same operation previewing on 3.10 and refused on 3.12, nine code points flipping,
the older interpreter the permissive side. **It is fixed, not accepted.** The class is now a
committed table derived from one published Unicode release
(`src/gramps_agent_data_entry/core/_unrenderable.py`, generated by `scripts/derive_unrenderable.py`,
provenance in `docs/schema-render-guard-derivation.md`), and it is identical on 3.10.20, 3.11.15
and 3.12.13 -- 143,789 code points, one membership digest, measured by absolute interpreter path
rather than inferred. Divergence **0**, against 9 before. What the fix costs instead is recorded
in the guard's own costs block, which moved with it to
`src/gramps_agent_data_entry/core/render_guard.py` when `core/schema.py` was deleted with the note flow;
it is not moved into this table. **A residual row left standing reads as current when it is not** -- the same reason
the row below was removed.
...
| Row | Result |
| --- | --- |
| tip — `python -m gramps_agent_data_entry.core.pii_guard .` | 0 findings over 45 tracked entries |
...
| Row | Result |
| --- | --- |
| tip — `python -m gramps_agent_data_entry.core.pii_guard .` | 0 findings over 45 tracked entries, exit 0 |
...
a runner that has never seen Gramps, and only `gramps_plugin/AgentDataEntry_writer.py` speaks to
...
deleted both with the note flow. The host's own code lives in `src/gramps_agent_data_entry/host/`, which
...
`gramps_plugin/AgentDataEntry.py` imports `gi` INSIDE `load_on_reg` rather than at module
...
`src/gramps_agent_data_entry/host/accessor.py`. Every public function in that module refuses a non-main
...
⚠️ **This is not probe-specific.** `AgentDataEntry.py` connects to the same signal in
