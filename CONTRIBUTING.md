...
(`src/gramps_live_api/core/_unrenderable.py`, generated by `scripts/derive_unrenderable.py`,
provenance in `docs/schema-render-guard-derivation.md`), and it is identical on 3.10.20, 3.11.15
and 3.12.13 -- 143,787 code points, one membership digest, measured by absolute interpreter path
...
`src/gramps_live_api/core/render_guard.py` when `core/schema.py` was deleted with the note flow;
